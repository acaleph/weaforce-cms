...
$(document).ready(function(){
	...
	//检索当前文章的关联文章'
	albumList($('#adsId').val(),$('#serviceId').val(),$('#queryName').val(),$('#queryDateFrom').val(),$('#queryDateTo').val());
})
albumList = function (adsId,serviceId,albumName,dateFrom,dateTo){
	$.ajax({
		type:"post",
		dataType:"json",
		url:"/cms/ads/ads-service!getAlbumListJSON.action",
		data:{'adsId':adsId,'serviceId':serviceId,'queryName':albumName,'queryDateFrom':dateFrom,'queryDateTo':dateTo},
		success:function(data){
			...
			$.each(data,function(index,item){
				...
				if (item['checked'] == 0)
					row.find("#albumId").html("<input type=\"checkbox\" id=\"checkedAlbumIds\" name=\"checkedArticleIds\" value=\"" + item['albumId'] + "\" onClick=\"saveAlbum("+ $('#adsId').val() + "," +$('#serviceId').val() + "," + item['albumId'] + ")\">");
				if (item['checked'] == 1)
					row.find("#albumId").html("<input type=\"checkbox\" id=\"checkedAlbumIds\" name=\"checkedArticleIds\" value=\"" + item['albumId'] + "\" onClick=\"saveAlbum("+ $('#adsId').val() + "," +$('#serviceId').val() + "," + item['albumId'] + ")\" checked=\"true\">");
...
//保存关联文章
saveAlbum = function(adsId,serviceId,albumId){
	//alert("primaryId: " + primaryId + " slaveId: " + slaveId);
	$.ajax({
		type:"post",
		dataType:"json",
		url:"/cms/ads/ads-service!saveAlbum.action",
		data:{'adsId':adsId,'serviceId':serviceId,'albumId':albumId},
		success:function(data){
			$.each(data,function(index,item){
...
<table border="0" cellpadding="0" cellspacing="1" id="searchbg" align="center" width="98%">
	<tr align="center">
		...
		<td><input type="button" id="search" name="search" value="Search" onClick="javascript:albumList($('#adsId').val(),$('#serviceId').val(),$('#queryName').val(),$('#queryDateFrom').val(),$('#queryDateTo').val());"></td>
	</tr>
</table>
<div class="spacer-2"></div>
<table id="table-stripe" border="0" cellpadding="0"  cellspacing="1" class="stripe" align="center" width="98%">
	...
	<tr>
		<td colspan="3">
			...
			<input type="hidden" id="adsServiceAlbum" name="adsServiceAlbum" value="<s:property value='adsServiceAlbum' />" >
		</td>
	</tr>
	<tr id="template">
